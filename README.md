...
We provide 2 run configurations for IntelliJ Idea:

- `AtotiSpringBootApplication`: uses the default `application.yml` which disables the OpenTelemetry.
- `AtotiSpringBootApplication with OTEL`: uses the profile `otel`(`application-otel.yml`) which activates the OpenTelemetry. It will send the metrics and logs to the OTEL collector at `http://localhost:4318`, hence you should start the OTEL stack otherwise you would see the exceptions.
